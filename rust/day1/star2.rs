...
fn main() {
    ...
    for val in it {
        match *val {
            d if d >= first => {third = second; second = first; first = *val;},
            d if d >= second => {third = second; second = *val;},
            d if d >= third => {third = *val},
            _ => (),
        }
    }

    print!("first: {} second: {} third: {}\n sum equals: {}", first, second, third, (first+second+third))
}